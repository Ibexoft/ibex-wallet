                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('transactions.update', ['transaction' => $transaction->id]) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required autocomplete="type" autofocus>
                                    <option value="1" {{ $transaction->type == '1' || old('type') == '1' ? 'selected' : ''}}>Expense</option>
                                    <option value="2" {{ $transaction->type == '2' || old('type') == '2' ? 'selected' : ''}}>Income</option>
                                    <option value="3" {{ $transaction->type == '3' || old('type') == '3' ? 'selected' : ''}}>Transfer</option>
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="amount" type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ $transaction->amount }}" required autocomplete="amount">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="details" type="text" class="form-control @error('details') is-invalid @enderror" name="details" value="{{ $transaction->details }}" autocomplete="details">
                            ...
                            <div class="col-md-6">
                                <select name="src_account_id" id="src_account_id" class="form-control @error('src_account_id') is-invalid @enderror" required autocomplete="src_account_id">
                                    ...
                                    @foreach ($accounts as $account)
                                        <option value="{{ $account->id }}" {{ $transaction->src_account->id == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select name="dest_account_id" id="dest_account_id" class="form-control @error('dest_account_id') is-invalid @enderror" autocomplete="dest_account_id">
                                    <option></option>
                                    @foreach ($accounts as $account)
                                        <option value="{{ $account->id }}" {{ $transaction->dest_account && $transaction->dest_account->id == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="wallet_id" id="wallet_id" class="form-control @error('wallet_id') is-invalid @enderror" required autocomplete="wallet_id">
                                    ...
                                    @foreach ($wallets as $wallet)
                                        <option value="{{ $wallet->id }}" {{ $transaction->wallet->id == $wallet->id ? 'selected' : '' }}>{{ $wallet->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="spent_on" type="text" class="form-control @error('spent_on') is-invalid @enderror" name="spent_on" value="{{ $transaction->spent_on }}" autocomplete="spent_on">
